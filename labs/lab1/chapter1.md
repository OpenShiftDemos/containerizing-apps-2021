...
Here we are just going to explore a simple Dockerfile. The purpose for this is to have a look at some of the basic commands that are used to construct a podman or docker image. For this lab, we will explore a basic Apache httpd Dockerfile and then confirm functionality. To be clear, the filename is not required to be `Dockerfile` however, if you use that name or `Containerfile` `podman` will find it automatically.
...
Remember, you can always install what you need while you are debugging something. However, it won't be there on the next start of the container unless you add it to your Dockerfile. For example:
